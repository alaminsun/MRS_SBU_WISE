...
{
    public class RptSelectedDoctorValueShareController : Controller
    {
        ...
        public RptSelectedDoctorValueShareController()
        {
            _vmMsg = new ValidationMsg();
            _doctorGroupMstDtlDao = new DoctorGroupMstDtlDAO();
        }
        public ActionResult frmRptSelectedDoctorValueShare()
        {
            //if (Session["UserId"] != null)
            //{
            ViewBag.formTitle = "Manufacturer Wise Selected Doctor Value Share";
            return View();
            //}
        ...
        [HttpPost]
        public ActionResult frmRptSelectedDoctorValueShare(ReportModel model)
        {
            model.doctorList = (List<DoctorGroupDtlModel>)Session["doctorList"]; 
            DataTable dt;
            ReportDocument reportDocument = new ReportDocument();
            dt = Dalobject.SelectedDoctorValueShare(model);
            string ReportPath = Server.MapPath("~/Reports");

            switch (model.ReportName)
            {
                case "MANUFACTURER WISE SELECTED DOCTOR VALUE SHARE(By %)":

                    ReportPath = ReportPath + "/MANUFACTURER WISE SELECTED DOCTOR VALUE SHARE (by perc).rpt";
                    break;
                default:
                    ReportPath = ReportPath + "/MANUFACTURER WISE SELECTED DOCTOR VALUE SHARE (by Val).rpt";
                    break;
            }
