...
namespace MRS.DAL.DAO
{
    public class OTCInfoDAO
    {
        ...
        public List<OTCInfoModel> GetMarketInfoList()
        {
            string Qry = "";
            Qry = "SELECT * FROM MARKET ORDER BY MARKET_CODE";

            DataTable dt = _dbHelper.GetDataTable(Qry);
            List<OTCInfoModel> items;
            items = (from DataRow row in dt.Rows
                     select new OTCInfoModel
                     {
                         OT_MARKET_CODE = row["MARKET_CODE"].ToString(),
                         OT_MARKET_NAME = row["MARKET_NAME"].ToString()
                     }).ToList();

            return items;
        }

        public List<OTCInfoDetailModel> GetProductInfoList()
        {
            ...
            DataTable dt = _dbHelper.GetDataTable(Qry);
            List<OTCInfoDetailModel> items;
            items = (from DataRow row in dt.Rows
                     select new OTCInfoDetailModel
                     {
        ...
        public ValidationMsg Save(OTCInfoModel master, string userid)
        {

            try
            {
                //string OTCCategory = "T";
                //master.PRESC_CATE_CODE = OTCCategory;

                ////_vmMsg = new ValidationMsg();
                //master.PRESC_MAS_SLNO = idGenerated.getMAXSL("PRESC_MAS_SLNO", "PRESCRIPTION_MASTER");

                //if (master.OTCInfoDetailModels.Count() > 0)
                //{
                //    //MstSL = GetMstSLNO();
                ...
                //    foreach (var detailModel in master.OTCInfoDetailModels)
                //    {
                ...
                if (IsMSTExitsByInstituteCode(master.OT_MARKET_CODE))
                {
                    if (master.OTCInfoDetailModels.Count() > 0)
                    {
                        string mstSL = GetMstSLNO(master.OT_MARKET_CODE);
                        master.OT_PRESC_MAS_SLNO = Convert.ToInt64(mstSL);
                        ...
                        foreach (var detailModel in master.OTCInfoDetailModels)
                            ...
                            {
                                string query = "Update PRESCRIPTION_DETAIL Set PROD_ID='" + detailModel.PROD_ID + "', PURCHASE_QTY='" + detailModel.PURCHASE_QTY + "', UNIT_PRICE='" + detailModel.UNIT_PRICE + "'  WHERE PRESC_DET_SLNO = '" + detailModel.OT_PRESC_DET_SLNO + "'";
                            ...
                            {
                                detailModel.OT_PRESC_DET_SLNO = idGenerated.getMAXSL("PRESC_DET_SLNO", "PRESCRIPTION_DETAIL");
                                string qry1 = "INSERT INTO PRESCRIPTION_DETAIL(PRESC_MAS_SLNO, PRESC_DET_SLNO, PROD_ID, PURCHASE_QTY, UNIT_PRICE) VALUES('" + master.OT_PRESC_MAS_SLNO + "', '" + detailModel.OT_PRESC_DET_SLNO + "', '" + detailModel.PROD_ID + "', '" + detailModel.PURCHASE_QTY + "','" + detailModel.UNIT_PRICE + "' )";
                ...
                {
                    string OTCCategory = "T";
                    master.OT_PRESC_CAT_CODE = OTCCategory;
                    master.OT_PRESC_MAS_SLNO = idGenerated.getMAXSL("PRESC_MAS_SLNO", "PRESCRIPTION_MASTER");
                    if (master.OTCInfoDetailModels.Count() > 0)
                    {
                        string qry = "INSERT INTO PRESCRIPTION_MASTER(PRESC_MAS_SLNO,MARKET_CODE,SESSION_SLNO,PRESC_CATE_CODE) VALUES(" + master.OT_PRESC_MAS_SLNO + ", '" + master.OT_MARKET_CODE + "'," + HttpContext.Current.Session["SessionSl"] + ",'" + master.OT_PRESC_CAT_CODE + "')";
                        _dbHelper.CmdExecute(qry);
                        foreach (var detailModel in master.OTCInfoDetailModels)
                        {
                            detailModel.OT_PRESC_DET_SLNO = idGenerated.getMAXSL("PRESC_DET_SLNO", "PRESCRIPTION_DETAIL");
                            qry = "INSERT INTO PRESCRIPTION_DETAIL(PRESC_MAS_SLNO, PRESC_DET_SLNO, PROD_ID, PURCHASE_QTY, UNIT_PRICE) VALUES('" + master.OT_PRESC_MAS_SLNO + "', '" + detailModel.OT_PRESC_DET_SLNO + "', '" + detailModel.PROD_ID + "', '" + detailModel.PURCHASE_QTY + "','" + detailModel.UNIT_PRICE + "' )";
                            _dbHelper.CmdExecute(qry);
                        }
                        _validationMsg.Type = Enums.MessageType.Success;
                        _validationMsg.Msg = "Save Successful.";
        ...
        public ValidationMsg Update(OTCInfoModel master, string userid)
        {
            try
            {
                //int? MstSL;
                ...
                foreach (var detailModel in master.OTCInfoDetailModels)
                {
                    if (detailModel.OT_PRESC_DET_SLNO == 0)
                    {
                        string prescriptionMstSl = GetMstSLNO(master.OT_MARKET_CODE);
                        detailModel.OT_PRESC_DET_SLNO = idGenerated.getMAXSL("PRESC_DET_SLNO", "PRESCRIPTION_DETAIL");
                        string qry = "INSERT INTO PRESCRIPTION_DETAIL(PRESC_MAS_SLNO, PRESC_DET_SLNO, PROD_ID, PURCHASE_QTY, UNIT_PRICE) VALUES('" + prescriptionMstSl + "', '" + detailModel.OT_PRESC_DET_SLNO + "', '" + detailModel.PROD_ID + "', '" + detailModel.PURCHASE_QTY + "','" + detailModel.UNIT_PRICE + "' )";
                        _dbHelper.CmdExecute(qry);
                    }
                    else
                    {
        ...
        public List<OTCInfoModel> GetMarketList()
        {
            string Qry = "";
            ...
            items = (from DataRow row in dt.Rows
                     select new OTCInfoModel
                     {
                         OT_PRESC_MAS_SLNO = Convert.ToInt64(row["PRESC_MAS_SLNO"].ToString()),
                         SessionSl = Convert.ToInt64(row["SESSION_SLNO"].ToString()),
                         OT_PRESC_CAT_CODE = row["PRESC_CATE_CODE"].ToString(),
                         OT_MARKET_CODE = row["MARKET_CODE"].ToString(),
                         OT_MARKET_NAME = row["MARKET_NAME"].ToString()
                     }).ToList();
        ...
        public List<OTCInfoDetailModel> GetProductList()
        {
            string Qry = "";
            ...
            items = (from DataRow row in dt.Rows
                     select new OTCInfoDetailModel
                     {
                         OT_PRESC_MAS_SLNO = Convert.ToInt16(row["PRESC_MAS_SLNO"].ToString()),
                         OT_PRESC_DET_SLNO = Convert.ToInt16(row["PRESC_DET_SLNO"].ToString()),
                         PROD_ID = row["PROD_ID"].ToString(),
                         DOSAGE_FORM_CODE = row["DOSAGE_FORM_CODE"].ToString(),
                         DOSAGE_FORM_NAME = row["DOSAGE_FORM_NAME"].ToString(),
                         DSG_STRENGTH_CODE = row["DSG_STRENGTH_CODE"].ToString(),
        ...
        public List<OTCInfoDetailModel> GetSearchProduct(string marketCode, string sessionSl)
        {
            using (OracleConnection con = new OracleConnection(dbCon.StringRead()))
            {
                ...
                OracleCommand cmd = new OracleCommand(query, con);
                List<OTCInfoDetailModel> products = new List<OTCInfoDetailModel>();
                con.Open();
                using (OracleDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        OTCInfoDetailModel model = new OTCInfoDetailModel();
                        model.OT_PRESC_DET_SLNO = Convert.ToInt64(reader["PRESC_DET_SLNO"]);
                        model.OT_PRESC_MAS_SLNO = Convert.ToInt64(reader["PRESC_MAS_SLNO"]);
        ...
        public List<OTCInfoModel> GetSessionMarketData(long sessionSl)
        {
            using (OracleConnection con = new OracleConnection(dbCon.StringRead()))
            {
                string query = "Select M.PRESC_MAS_SLNO,M.MARKET_CODE,M.PRESC_CATE_CODE,MD.MARKET_NAME " +
                               "from   PRESCRIPTION_MASTER M " +
                               "Inner Join MARKET MD on M.MARKET_CODE = MD.MARKET_CODE " +
                               "Where  M.SESSION_SLNO=" + sessionSl + " AND PRESC_CATE_CODE = 'T' ";
                OracleCommand cmd = new OracleCommand(query, con);
                List<OTCInfoModel> markets = new List<OTCInfoModel>();
                con.Open();
                using (OracleDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        OTCInfoModel model = new OTCInfoModel();
                        model.OT_MARKET_CODE = reader["MARKET_CODE"].ToString();
                        model.OT_MARKET_NAME = reader["MARKET_NAME"].ToString();
                        model.OT_PRESC_MAS_SLNO = Convert.ToInt64(reader["PRESC_MAS_SLNO"].ToString());
                        model.OT_PRESC_CAT_CODE = reader["PRESC_CATE_CODE"].ToString();
